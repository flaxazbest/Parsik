...
import ua.azbest.part.OldVideocard;
...
public class OldVideoParser {

    private LinkedList<String> links;
    private LinkedList<OldVideocard> oldVideocards;
    private HashSet<String> chipsets;

    public OldVideoParser() {
        links = new LinkedList<>();
        oldVideocards = new LinkedList<>();
    ...
    public HashSet<String> getChipsetsFromFile() {
        ...
        LinkedList<OldVideocard> vc =  new LinkedList<>();

        try (FileInputStream streamIn = new FileInputStream("oldVideocards.dat");
             ObjectInputStream objectinputstream = new ObjectInputStream(streamIn)) {

            OldVideocard readCase = null;
            do {
                readCase = (OldVideocard) objectinputstream.readObject();
                if(readCase != null){
                    vc.add(readCase);
                }
            } while (readCase != null);
    ...
    public void parse() {
        ...
        String firstPage = "http://hard.rozetka.com.ua/oldVideocards/c80087/";
        Document doc = null;
        try {
           ...
           // for (int i=0; i<references.size(); i++) {
            for (int i=0; i<4; i++) {
                ...
                if (newsHeadlines != null) {
                    ...
                    OldVideocard v = new OldVideocard(fullName, manufacture, p, det);
                    oldVideocards.add(v);
            ...
            System.out.println("===============");
            //System.out.println(oldVideocards);
            ...
            try{
                FileOutputStream fileOut = new FileOutputStream("oldVideocards.dat", true);
                ObjectOutputStream ooos = new ObjectOutputStream(fileOut);
                for (OldVideocard v: oldVideocards)
                    ooos.writeObject(v);
            } catch (Exception ex) {
                ex.printStackTrace();
            } finally {
